"""
LangChain JSON Agent 详细注释版示例

本示例展示了如何创建和使用JSON格式的智能代理，包括：
1. 环境配置和错误处理 - 安全地加载API密钥和配置参数
2. 搜索工具集成 - 集成SerpAPI实现网络搜索功能
3. JSON代理创建和使用 - 使用LangChain的JSON代理框架
4. 完整的对话交互示例 - 提供实际可用的对话界面

JSON代理工作原理：
- JSON代理使用结构化的JSON格式与工具交互
- 代理通过JSON对象调用工具，包含action和action_input字段
- 工具返回结果后，代理生成最终的JSON响应
...
from langchain.agents import create_json_chat_agent  # JSON聊天代理创建函数
...
def create_agent_executor(config):
    ...
    - create_json_chat_agent: 创建使用JSON格式的聊天代理
    ...
    try:
        ...
        # 使用LangChain Hub的JSON代理提示词模板
        # 这个模板已经包含了所有必需的变量
        try:
            prompt = hub.pull("hwchase17/react-chat-json")
            logger.info("从Hub加载JSON代理提示词模板成功")
        except Exception as e:
            logger.warning(f"从Hub加载提示词失败: {e}，使用自定义模板")

            # 创建自定义的JSON代理提示词模板
            # 确保包含所有必需的变量
            system_message = """你是一个有用的AI助手，可以使用工具来回答问题。
...
工具调用示例：
- 搜索信息：{{"action": "search", "action_input": "今天北京天气"}}
- 数学计算：{{"action": "calculator", "action_input": "2+3*4"}}

请根据用户的问题，决定是否需要使用工具，并提供准确的回答。"""
            ...
            logger.info("自定义JSON代理提示词模板创建成功")
        ...
        # 创建JSON聊天代理
        # JSON代理使用结构化的JSON格式来调用工具和生成回答
        # 格式示例：{"action": "search", "action_input": "查询内容"}
        agent = create_json_chat_agent(
            llm=llm,        # 语言模型
            tools=tools,    # 可用工具列表
            prompt=prompt   # 提示词模板
        )
        logger.info("JSON代理创建成功")
...
def query_agent(agent_executor, question: str, verbose: bool = True) -> Dict[str, Any]:
    ...
    用户问题: "今天北京的天气怎么样？"
    代理思考: 需要获取实时天气信息
    工具调用: {"action": "search", "action_input": "今天北京天气"}
    搜索结果: 天气数据...
    最终回答: 今天北京天气晴朗...

    用户问题: "计算 15 * 8 + 32"
    代理思考: 需要进行数学计算
    工具调用: {"action": "calculator", "action_input": "15*8+32"}
    计算结果: 152
    最终回答: 计算结果是152
    """
    try:
...
def main():
    ...
    print("=" * 60)
    print("LangChain JSON Agent 详细注释版示例")
    print("=" * 60)
    print("本程序演示如何使用LangChain创建JSON格式的智能代理")
